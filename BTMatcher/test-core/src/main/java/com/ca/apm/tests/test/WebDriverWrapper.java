...
public class WebDriverWrapper extends ElementsIdentification
{
    ...
    public Select select= null;
     ...
     public void hitTransactionCorId(String url)
     {
         initializeEMandAgents();
         fd1 = new FirefoxDriver();
         fd1.get(url);
         WebElement we = fd1.findElementByCssSelector("input[name='url']");
         we.clear();
         we.sendKeys("http://"+weblogicAgenthost1+":7001/QATestApp/urlhitter/ServletCallingJSP?duration=1000");
         WebElement we1 = fd1.findElementByCssSelector(" input[value='HIT IT']");
         we1.click();
         try
        {
            Thread.sleep(30000);
