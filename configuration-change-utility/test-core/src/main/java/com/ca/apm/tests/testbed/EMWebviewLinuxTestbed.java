...
@TestBedDefinition
public class EMWebviewLinuxTestbed implements ITestbedFactory{
    ...
    public static final String WORKSTAATION_ROLE_ID = "workstationRole";

    @Override
    public ITestbed create(ITasResolver tasResolver){
    	EmRole emRole = new EmRole.LinuxBuilder(EM_ROLE_ID, tasResolver).nostartEM().nostartWV().build();
    	WorkstationRole wsRole = new WorkstationRole.Builder(WORKSTAATION_ROLE_ID, tasResolver).build();
    	
    	ITestbedMachine emMachine = TestBedUtils.createLinuxMachine(EM_MACHINE_ID, EM_MACHINE_TEMPLATE_ID, emRole,wsRole);
