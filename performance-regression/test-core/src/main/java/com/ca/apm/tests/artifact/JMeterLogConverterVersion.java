 ...
 */
public class JMeterLogConverterVersion implements ITasArtifactFactory {
    private static final String GROUP_ID = "com.ca.apm.coda-projects.test-tools";
    private static final String ARTIFACT_ID = "jmeterlogconverter";
    private static final IBuiltArtifact.TasExtension EXTENSION;
    private final ITasResolver resolver;

    static {
        EXTENSION = IBuiltArtifact.TasExtension.JAR;
    }

    public JMeterLogConverterVersion(ITasResolver resolver) {
        this.resolver = resolver;
    }

    public ITasArtifact createArtifact(String version) {
        return (new TasArtifact.Builder(ARTIFACT_ID)).groupId(GROUP_ID).version(version == null ? this.resolver.getDefaultVersion() : version).extension(EXTENSION).build();
    }
