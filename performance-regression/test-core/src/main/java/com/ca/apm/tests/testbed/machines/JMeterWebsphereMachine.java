 ...
 */
public class JMeterWebsphereMachine extends JMeterMachineAbs {

    public static final String DEFAULT_TEMPLATE = "AgentPerf_JMETERWEBSPHERE";
    ...
    public JMeterWebsphereMachine(String machineId, ITasResolver tasResolver, String sharePassword) {
        super(machineId, tasResolver, sharePassword);
    }

    public JMeterWebsphereMachine(String machineId, ITasResolver tasResolver, boolean undeploy, String sharePassword) {
    ...
    public TestbedMachine initPredeployed(Trade6Role trade6Role, IRole emRole) {
        this.predeployed = true;
        return init(trade6Role, emRole);
    }

    public TestbedMachine init(Trade6Role trade6Role, IRole emRole) {
        TestbedMachine machine = new TestbedMachine.Builder(machineId).templateId(DEFAULT_TEMPLATE).build();
        ...
        if (undeploy) {
            ...
            String jmxScriptFile = "trade6-jmeter-var.jmx";
...
//
            params.put("TRADE6.NUM.THREADS", String.valueOf(jmeterNumThreads));
            params.put("TRADE6.HOSTNAME", tasResolver.getHostnameById(trade6Role.getRoleId()));
            params.put("TRADE6.HTTP.PORT", String.valueOf(trade6Role.getPortNumber()));
            params.put("TRADE6.DELAY.BETWEEN.REQUESTS", String.valueOf(jmeterDelayBetweenRequests));
