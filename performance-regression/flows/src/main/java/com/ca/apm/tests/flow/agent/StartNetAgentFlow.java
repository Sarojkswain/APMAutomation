 ...
 */
@Flow
public class StartNetAgentFlow extends FlowBase {

    private static final Logger LOGGER = LoggerFactory.getLogger(StartNetAgentFlow.class);

    public StartNetAgentFlow() {
    }
    ...
    protected void startAgent() throws InterruptedException {
        int responseCode = this.getExecutionBuilder(LOGGER, "net")
                .args(new String[]{"start", "CA APM PerfMon Collector Service"}).build().go();
        switch (responseCode) {
            case 0:
