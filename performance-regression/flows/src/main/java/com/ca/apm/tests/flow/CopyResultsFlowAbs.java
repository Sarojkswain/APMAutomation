...
public abstract class CopyResultsFlowAbs extends FlowBase {
     ...
     */
    public static long copyStream(InputStream input, OutputStream output, boolean logAlive, @Nullable String fileName) throws IOException {
        long count = 0L;
        ...
        for (boolean n = false; -1 != (n1 = input.read(buffer)); count += (long) n1) {
            ...
            if (logAlive && currentTime - time > 20000) {
                ...
                LOGGER.info("Stream copy is alive." + (fileName != null ? "[" + fileName + "]" : ""));
                time = currentTime;
            }
        }
        return count;
    ...
    protected void configNet(String remoteMachine, String user, String password) throws InterruptedException {
        int responseCode = this.getExecutionBuilder(LOGGER, "net").args(new String[]{"use", remoteMachine,
                "/user:" + user, password}).build().go();
        switch (responseCode) {
            case 0:
                LOGGER.info("Net completed SUCCESSFULLY! Congratulations!");
                return;
            default:
                throw new IllegalStateException(String.format("Net failed (%d)", new Object[]{responseCode}));
        }
    }

    protected void copyFile(String origFile, String destFile) throws InterruptedException {
        int responseCode = this.getExecutionBuilder(LOGGER, "copy").args(new String[]{origFile, destFile}).build().go();
        switch (responseCode) {
            case 0:
                LOGGER.info("Copy File completed SUCCESSFULLY! Congratulations!");
                return;
            default:
                throw new IllegalStateException(String.format("Copy File failed (%d)", new Object[]{responseCode}));
        }
    }
