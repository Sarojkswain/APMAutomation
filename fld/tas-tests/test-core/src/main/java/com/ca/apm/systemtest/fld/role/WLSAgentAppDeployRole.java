...
public class WLSAgentAppDeployRole extends AbstractRole {
    ...
    private String pipeorganLogFile;
    private String pipeorganLoggingLevel;
    private boolean redirectStdoutToServerLogEnabled;
    ...
    private static final String WLS12C_INSTALL_HOME = DEPLOY_BASE + "Oracle" + SEPARATOR + "Middleware12.1.3";
    ...
    protected WLSAgentAppDeployRole(Builder builder) {
        ...
        this.pipeorganLogFile = builder.pipeorganLogFile;
        this.pipeorganLoggingLevel = builder.pipeorganLoggingLevel;
        this.redirectStdoutToServerLogEnabled = builder.redirectStdoutToServerLogEnabled;
    }
    ...
    private void startPODomain(IAutomationAgentClient aaClient) {
        RunCommandFlowContext ctx = new RunCommandFlowContext.Builder(ALT_START_SCRIPT)
            .workDir(PO_DOMAIN_HOME).doNotPrependWorkingDirectory()
            .terminateOnMatch(START_MATCH_TEXT)
            .build();
    ...
    private void waitForWebLogicStart() {
        ...
        while (!started && System.currentTimeMillis() < timeout) {
            try {
                ...
                // if no connection is made to WLS, an exception is thrown, which we don't care about
            }
    ...
    private void deployArtifacts(IAutomationAgentClient aaClient) {
        ...
        context = new GenericFlowContext.Builder()
           .artifactUrl(tasResolver.getArtifactUrl(new DefaultArtifact("com.ca.apm.binaries.weblogic", "pipeorgandomain", "zip", "10.3")))
    ...
    private void updatePODomainStartWLSCMD(IAutomationAgentClient aaClient) {
        ...
        replacePairs.put("\\[AGENT.JAVA.CLASSPATH\\]", PO_DOMAIN_HOME + "/wily/common/WebAppSupport.jar"); 
        replacePairs.put("\\[WLS.LOG.OUTPUT\\]", PO_DOMAIN_HOME + "/WebLogicConsole.log");
        replacePairs.put("\\[OTHER.JAVA.OPTIONS\\]", getOtherJavaOptions());
        ...
        Collection<String> appendLines = Arrays.asList(new String[] {
            "START \"WLS Rulez\" startWebLogic.cmd",
            "echo " + START_MATCH_TEXT
        });
        
        FileCreatorFlowContext createContext = new FileCreatorFlowContext.Builder()
            .destinationFilename(ALT_START_SCRIPT)
            .destinationDir(PO_DOMAIN_HOME)
            .fromData(appendLines)
            .build();
        runFlow(aaClient, FileCreatorFlow.class, createContext);
    }

    private String getOtherJavaOptions() {
        StringBuilder sb = new StringBuilder();
        if (StringUtils.hasText(pipeorganLogFile)) {
            sb.append(" -Dpipeorgan.logfile=").append(pipeorganLogFile);
        }
        if (StringUtils.hasText(pipeorganLoggingLevel)) {
            sb.append(" -Dpipeorgan.logging=").append(pipeorganLoggingLevel);
        }

        if (redirectStdoutToServerLogEnabled) {
            sb.append(" -Dweblogic.log.RedirectStdoutToServerLogEnabled=true");
        }
        return sb.toString();
    }
    ...
    private void updateCrossJVMTracing(IAutomationAgentClient aaClient) {
        ...
        replacePairs.put("#introscope.agent.weblogic.crossjvm=true", "introscope.agent.weblogic.crossjvm=true" );
    ...
    public static class Builder extends BuilderBase<Builder, WLSAgentAppDeployRole> {
        ...
        protected String pipeorganLogFile;
        ...
        public Builder pipeorganNoLog() {
            return pipeorganLogFile("NUL").pipeorganLoggingLevel("nologging");
        }

        public Builder pipeorganLogFile(String pipeorganLogFile) {
            this.pipeorganLogFile = pipeorganLogFile;
            return builder();
        }
        ...
        public Builder redirectStdoutToServerLog() {
            this.redirectStdoutToServerLogEnabled = true;
            return builder();
        }
    }
