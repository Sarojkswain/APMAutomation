...
public class DotNetAgentHttpConnLoadProvider
    ...
    private final FLDConfiguration fldConfig;
    ...
    public DotNetAgentHttpConnLoadProvider(FLDConfiguration fldConfig) {
        this.fldConfig = fldConfig;
        this.machineIds = DOTNET_HTTP_CONN_MACHINES;
        this.hostNames = Arrays.copyOfRange(DOTNET_HTTP_CONN_HOST_NAMES, 0, machineIds.length);
        this.machines = new ArrayList<ITestbedMachine>(machineIds.length);
        assert this.fldConfig != null;
        agent2EmConnectionType =
            Agent2EmConnectionType.fromString(fldConfig.getAgent2EmConnectionType());
        useAgentTruststore = fldConfig.isEmNeedClientAuth();
    }
    ...
    public void initTestbed(ITestbed testbed, ITasResolver tasResolver) {
        String emHost = tasResolver.getHostnameById(DOTNET_HTTP_CONN_EM_MOM_ROLE_ID);
        LOGGER.info("DotNetAgentHttpConnLoadProvider.initTestbed():: emHost = {}", emHost);
        for (int i = 0; i < machines.size(); i++) {
            initMachine(tasResolver, machines.get(i), hostNames[i], emHost);
        }
    }
    ...
    private void initMachine(ITasResolver tasResolver, ITestbedMachine machine,
        ...
        DotNetAppsDeployRole dotNetAppsRole =
            new DotNetAppsDeployRole.Builder(roleId(machineId, DOTNET_HTTP_CONN_APPS_ROLE_ID),
        ...
        DotNetAgentDeployRole.Builder dotNetAgentRoleBuilder =
            new DotNetAgentDeployRole.Builder(roleId(machineId, DOTNET_HTTP_CONN_AGENT_ROLE_ID),
                tasResolver).installDir(TasBuilder.WIN_SOFTWARE_LOC + "dotnet").isLegacyMode(false)
                .emHost(emHost).agentHostName(agentHostName).agentVersion(fldConfig.getEmVersion());
        ...
        IRole registerIISRole =
            machine.getRoleById(roleId(machineId, DOTNET_HTTP_CONN_IIS_REGISTER_ROLE_ID));
        IRole updatePortRole =
            machine.getRoleById(roleId(machineId, DOTNET_HTTP_CONN_IIS_UPDATE_PORT_ROLE_ID));
    ...
    private void addEnableWebServerRole(ITestbedMachine machine) {
        ...
        ExecutionRole enableIISRole =
            new ExecutionRole.Builder(machineId + "_" + DOTNET_HTTP_CONN_IIS_ENABLE_ROLE_ID).flow(
        ...
        ExecutionRole updatePortRole =
            new ExecutionRole.Builder(roleId(machineId, DOTNET_HTTP_CONN_IIS_UPDATE_PORT_ROLE_ID))
        ...
        ExecutionRole registerIISRole =
            new ExecutionRole.Builder(roleId(machineId, DOTNET_HTTP_CONN_IIS_REGISTER_ROLE_ID))
    ...
    private void addTnsRole(ITasResolver tasResolver, ITestbedMachine machine) {
        ...
        GenericRole distRole =
            new GenericRole.Builder(machineId + "_" + DOTNET_HTTP_CONN_ODP_NET_SCRIPTS_ROLE_ID,
        ...
        ExecutionRole tnsRole =
            new ExecutionRole.Builder(roleId(machineId, DOTNET_HTTP_CONN_COPY_TNS_FILE_ROLE_ID))
