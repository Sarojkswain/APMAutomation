...
/**
 * TIM recording sessions provider.
 * 
 * @author Alexander Sinyushkin (sinal04@ca.com)
 *
 */
public class FLDTIMRecordingSessionProvider implements FLDConstants, FLDLoadConstants, FldTestbedProvider {
    ...
    private int recordingDurationMillis = DURATION_24_HOURS_IN_MILLIS;//run TIM recording every 24 hours
    private ITestbedMachine machine;

    public FLDTIMRecordingSessionProvider() {
    }
    
    public FLDTIMRecordingSessionProvider(String tessHost, int tessPort, String tessUser,
	...
	public Collection<ITestbedMachine> initMachines() {
        machine = new TestbedMachine.Builder(TIM_SESSION_RECORDING_MACHINE_ID)
	...
	public void initTestbed(ITestbed testbed, ITasResolver tasResolver) {
	    ...
	    String clientIPName = clientIP;
        if (clientIPName == null) {
            clientIPName = tasResolver.getHostnameById(TIM01_ROLE_ID); 
        }
	    
	    RecordingSessionRole.Builder roleBuilder = new RecordingSessionRole.Builder(TIM_SESSION_RECORDING_ROLE_ID, 
	    		tasResolver)
