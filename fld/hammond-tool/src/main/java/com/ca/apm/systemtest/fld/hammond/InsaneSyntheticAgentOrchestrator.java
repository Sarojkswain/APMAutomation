...
public class InsaneSyntheticAgentOrchestrator {

    protected static final Random rand = new Random(1449224256546L);

    private ArrayList<InsaneSyntheticAgent> agents = new ArrayList<>();
    ...
    public void createAgents(int agentsCount, int offset) {
        try {
            for (int i = 0; i < agentsCount; i++) {
                agents.add(createAgent(AgentName.getAgentName("Superdomain|host" + (i + offset) + "|process" + (i + offset) + "|name" + (i + offset))));
            }
    ...
    private InsaneSyntheticAgent createAgent(AgentName agentName)
        throws IOException, BadlyFormedNameException {
        InsaneSyntheticAgent result;
        ...
        result = new InsaneSyntheticAgent(feedback, agentName);
    ...
    public boolean startPlayback() {
        ...
        for (InsaneSyntheticAgent agent : agents) {
            try {
                agent.start();
