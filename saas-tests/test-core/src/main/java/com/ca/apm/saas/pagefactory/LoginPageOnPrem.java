 ...
 *
 */
public class LoginPageOnPrem {
    
    protected long maxWait = 15000;
    private static final Logger logger = LoggerFactory.getLogger(LoginPageOnPrem.class);
    ...
    public LoginPageOnPrem(WebDriver driver){
        this.driver = driver;
    ...
    private void switchToFrame(String frame) throws Exception {
        ...
        while(attempts <= 5) {
            try {
                ...
                e.printStackTrace();
                SaaSUtils.logAllUIElements(driver);
                if(attempts == 5) {
                    throw e;
                }
            }
