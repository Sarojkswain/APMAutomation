...
/**
 * The testbed is for deployment of standalone , Cluster with three collectors and 4 agents in Linux for
 * execution of all AgentControllability test cases. *
 */

@TestBedDefinition
public class ACCLinuxTestbed implements ITestbedFactory {
	...
	protected static String EM_TEMPLATE_ID = AccConstants.EM_TEMPLATE_ID_LINUX;

	public static final String AGENT_MACHINE_ID = AccConstants.AGENT_MACHINE_ID;
	private static final String AGENT_MACHINE_TEMPLATE_ID = AccConstants.AGENT_MACHINE_TEMPLATE_ID_LINUX;
	...
	public ITestbed create(ITasResolver tasResolver) {
		...
		List<String> collectorChosenFeatures = Arrays.asList(
				"Enterprise Manager", "WebView");

		// Collector1 role
		EmRole collector1Role = new EmRole.LinuxBuilder(COLLECTOR1_ROLE_ID,
				tasResolver)
		...
		// Collector2 role
		EmRole collector2Role = new EmRole.LinuxBuilder(COLLECTOR2_ROLE_ID,
				tasResolver)
				.emClusterRole(DeployEMFlowContext.EmRoleEnum.COLLECTOR)
				.silentInstallChosenFeatures(collectorChosenFeatures)
				.dbhost(tasResolver.getHostnameById(MOM_ROLE_ID)).nostartEM()
		...
		// Collector3 role
		EmRole collector3Role = new EmRole.LinuxBuilder(COLLECTOR3_ROLE_ID,
				tasResolver)
				.emClusterRole(DeployEMFlowContext.EmRoleEnum.COLLECTOR)
				.silentInstallChosenFeatures(collectorChosenFeatures)
				.dbhost(tasResolver.getHostnameById(MOM_ROLE_ID)).nostartEM()
		...
		// MOM role
		EmRole momRole = new EmRole.LinuxBuilder(MOM_ROLE_ID, tasResolver)
				.emClusterRole(DeployEMFlowContext.EmRoleEnum.MANAGER)
				.emCollector(collector1Role).emCollector(collector2Role)
				.emCollector(collector3Role).nostartEM().nostartWV().build();
		
		// Standalone role
        EmRole standaloneRole =
            new EmRole.LinuxBuilder(STANDALONE_ROLE_ID, tasResolver)        	
                .nostartEM().nostartWV().build();

		// QAAppRole for JBoss
		WebAppRole<JbossRole> qaAppJbossRole = new QaAppJbossRole.Builder(
				QA_APP_JBOSS_ROLE_ID, tasResolver).cargoDeploy()
				.contextName("qa-app").build();

		// QAAppRole for Tomcat
		WebAppRole<TomcatRole> qaAppTomcatRole = new QaAppTomcatRole.Builder(
				QA_APP_TOMCAT_ROLE_ID, tasResolver).cargoDeploy()
				.contextName("qa-app").build();

		// Jboss Role
		JbossRole jbossRole = new JbossRole.LinuxBuilder(JBOSS_ROLE_ID,
				tasResolver).version(JBossVersion.JBOSS711)
				.addWebAppRole(qaAppJbossRole).build();

		// Jboss Agent Role
		IRole jbossAgentRole = new AgentRole.LinuxBuilder(JBOSS_AGENT_ROLE_ID,
				tasResolver).webAppServer(jbossRole)
				.intrumentationLevel(AgentInstrumentationLevel.FULL)
				.emRole(momRole).build();

		// Tomcat Role
		TomcatRole tomcatRole = new TomcatRole.LinuxBuilder(TOMCAT_ROLE_ID,
				tasResolver).tomcatVersion(TomcatVersion.v70)
				.tomcatCatalinaPort(9091).webApp(qaAppTomcatRole).build();

		// Tomcat Agent Role
		IRole tomcatAgentRole = new AgentRole.LinuxBuilder(
				TOMCAT_AGENT_ROLE_ID, tasResolver).webAppServer(tomcatRole)
				.intrumentationLevel(AgentInstrumentationLevel.FULL)
				.emRole(momRole).build();
		
		// QAAppRole for JBoss1
		WebAppRole<JbossRole> qaAppJboss1Role = new QaAppJbossRole.Builder(
				QA_APP_JBOSS1_ROLE_ID, tasResolver).cargoDeploy()
				.contextName("qa-app").build();

		// QAAppRole for Tomcat1
		WebAppRole<TomcatRole> qaAppTomcat1Role = new QaAppTomcatRole.Builder(
				QA_APP_TOMCAT1_ROLE_ID, tasResolver).cargoDeploy()
				.contextName("qa-app").build();

		// Jboss1 Role
		JbossRole jboss1Role = new JbossRole.LinuxBuilder(JBOSS1_ROLE_ID,
				tasResolver).version(JBossVersion.JBOSS711)
				.addWebAppRole(qaAppJboss1Role).build();

		// Jboss1 Agent Role
		IRole jboss1AgentRole = new AgentRole.LinuxBuilder(JBOSS1_AGENT_ROLE_ID,
				tasResolver).webAppServer(jboss1Role).customName(JBOSS1_AGENT_NAME)
		    	.intrumentationLevel(AgentInstrumentationLevel.FULL)
				.emRole(momRole).build();

		// Tomcat1 Role
		TomcatRole tomcat1Role = new TomcatRole.LinuxBuilder(TOMCAT1_ROLE_ID,
				tasResolver).tomcatVersion(TomcatVersion.v70)
				.tomcatCatalinaPort(9091).webApp(qaAppTomcat1Role).build();

		// Tomcat1 Agent Role
		IRole tomcat1AgentRole = new AgentRole.LinuxBuilder(
				TOMCAT1_AGENT_ROLE_ID, tasResolver).webAppServer(tomcat1Role)
				.customName(TOMCAT1_AGENT_NAME).intrumentationLevel(AgentInstrumentationLevel.FULL)
				.emRole(momRole).build();
	
		
		// Configuration of Collector1 machine
		TestbedMachine collector1Machine = TestBedUtils.createLinuxMachine(
				COLLECTOR1_MACHINE_ID, EM_TEMPLATE_ID, collector1Role);

		// Configuration of Collector2 machine
		TestbedMachine collector2Machine = TestBedUtils.createLinuxMachine(
				COLLECTOR2_MACHINE_ID, EM_TEMPLATE_ID, collector2Role);

		// Configuration of Collector3 machine
		TestbedMachine collector3Machine = TestBedUtils.createLinuxMachine(
				COLLECTOR3_MACHINE_ID, EM_TEMPLATE_ID, collector3Role);

		// Configuration of mom machine
		TestbedMachine momMachine = TestBedUtils.createLinuxMachine(
				MOM_MACHINE_ID, EM_TEMPLATE_ID, momRole);
		
		 // Configuration of standalone machine
        TestbedMachine standaloneMachine =
            TestBedUtils.createLinuxMachine(STANDALONE_MACHINE_ID, EM_TEMPLATE_ID, standaloneRole,
            		tomcat1Role, qaAppTomcat1Role, tomcat1AgentRole, jboss1Role, qaAppJboss1Role,
                    jboss1AgentRole);

		// Configuration of Agent Machine
		TestbedMachine agentMachine = TestBedUtils.createLinuxMachine(
				AGENT_MACHINE_ID, AGENT_MACHINE_TEMPLATE_ID, tomcatRole,
				qaAppTomcatRole, tomcatAgentRole, jbossRole, qaAppJbossRole,
				jbossAgentRole);
        ...
        return Testbed.create(this, momMachine, collector1Machine,
				collector2Machine, collector3Machine, agentMachine, standaloneMachine);
