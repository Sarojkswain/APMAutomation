 ...
 * 
 * Author : KETSW01/ SWETHA KETHIREDDY
 * Date : 28/07/2016
 */
...
import com.ca.apm.tests.javascriptcalculators.JavaScriptCalculatorsConstants;
 ...
 * in Windows for execution of all JavaScriptCalculatorsMOM test cases. *
 */

@TestBedDefinition
public class JavaScriptCalculatorsMOMWindowsTestbed implements ITestbedFactory {

    public static final String JAVASCRIPTFILES_ROLE_ID =
        JavaScriptCalculatorsConstants.JAVASCRIPTFILES_ROLE_ID;
    private static final String JAVASCRIPTFILES_ARTIFACT_VERSION =
        JavaScriptCalculatorsConstants.JAVASCRIPTFILES_ARTIFACT_VERSION;
    private static final String JAVASCRIPTFILES_LOC =
        JavaScriptCalculatorsConstants.JAVASCRIPTFILES_LOC_WIN;
    ...
    public ITestbed create(ITasResolver tasResolver) {
        ...
        GenericRole downloadJavaScriptFilesRole =
            new GenericRole.Builder(JAVASCRIPTFILES_ROLE_ID, tasResolver).unpack(
                new DefaultArtifact("com.ca.apm.coda.testdata.em.javaScriptCalculators",
                    "javaScriptFiles", "zip", JAVASCRIPTFILES_ARTIFACT_VERSION),
                JAVASCRIPTFILES_LOC).build();


        // Collector1 role
        EmRole collector1Role =
            new EmRole.Builder(AgentControllabilityConstants.COLLECTOR1_ROLE_ID, tasResolver)
        ...
        // MOM role
        EmRole momRole =
            new EmRole.Builder(AgentControllabilityConstants.MOM_ROLE_ID, tasResolver)
        ...
        TestbedMachine collector1Machine =
            TestBedUtils.createWindowsMachine(AgentControllabilityConstants.COLLECTOR1_MACHINE_ID,
                AgentControllabilityConstants.WINDOWS_TEMPLATE_ID, collector1Role);

        // Configuration of mom machine
        TestbedMachine momMachine =
            TestBedUtils.createWindowsMachine(AgentControllabilityConstants.MOM_MACHINE_ID,
                AgentControllabilityConstants.WINDOWS_TEMPLATE_ID, momRole,
                downloadJavaScriptFilesRole);
        ...
        TestbedMachine agentMachine =
            TestBedUtils.createWindowsMachine(AgentControllabilityConstants.TOMCAT_MACHINE_ID,
                AgentControllabilityConstants.WINDOWS_TEMPLATE_ID, tomcatRole, qaAppTomcatRole,
                tomcatAgentRole);
        ...
        momMachine.addRemoteResource(RemoteResource.createFromRegExp(".*_(\\d+)", momRole
            .getDeployEmFlowContext().getInstallDir() + "\\config"));
        collector1Machine.addRemoteResource(RemoteResource.createFromRegExp(".*_(\\d+)",
            collector1Role.getDeployEmFlowContext().getInstallDir() + "\\config"));
        agentMachine.addRemoteResource(RemoteResource.createFromRegExp(".*_(\\d+)", tomcatRole
            .getTomcatFlowContext().getTomcatInstallDir() + "\\wily\\core\\config"));

        return Testbed.create(this, collector1Machine, momMachine, agentMachine);
