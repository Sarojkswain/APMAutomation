...
@TestBedDefinition
public class AgentLoadBalancingLinuxTestbed implements ITestbedFactory {
    ...
    public ITestbed create(ITasResolver tasResolver) {
        ...
        IRole jbossAgentRole =
            new AgentRole.LinuxBuilder(JBOSS_AGENT_ROLE_ID, tasResolver).webAppRole(jbossRole)
        ...
        IRole tomcatAgentRole =
            new AgentRole.LinuxBuilder(TOMCAT_AGENT_ROLE_ID, tasResolver).webAppRole(tomcatRole)
                .intrumentationLevel(AgentInstrumentationLevel.FULL).emRole(momRole)
                .build();
        ...
        TestbedMachine collector1Machine =
            TestBedUtils
                .createLinuxMachine(COLLECTOR1_MACHINE_ID, EM_TEMPLATE_ID, collector1Role);

        // Configuration of Collector2 machine
        TestbedMachine collector2Machine =
            TestBedUtils
                .createLinuxMachine(COLLECTOR2_MACHINE_ID, EM_TEMPLATE_ID, collector2Role);

        // Configuration of mom machine
        TestbedMachine momMachine =
            TestBedUtils.createLinuxMachine(MOM_MACHINE_ID, EM_TEMPLATE_ID, momRole);
