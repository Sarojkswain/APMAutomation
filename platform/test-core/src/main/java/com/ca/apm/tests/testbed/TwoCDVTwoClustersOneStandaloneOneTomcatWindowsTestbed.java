...
@TestBedDefinition
public class TwoCDVTwoClustersOneStandaloneOneTomcatWindowsTestbed implements ITestbedFactory {
    ...
    public ITestbed create(ITasResolver tasResolver) {
        ...
        //CDV1 role
        EmRole cdv1Role = new EmRole.Builder(CDV1_ROLE_ID, tasResolver)
        .emClusterRole(DeployEMFlowContext.EmRoleEnum.CDV)
        .silentInstallChosenFeatures(emChosenFeatures)
        .installDir(TasBuilder.WIN_SOFTWARE_LOC+"cdv1")
        .emCollector(mom1Col1Role)
        .emCollector(mom2Col1Role)
        .nostartEM().nostartWV().build();

      //CDV2 role
        EmRole cdv2Role = new EmRole.Builder(CDV2_ROLE_ID, tasResolver)
        .emClusterRole(DeployEMFlowContext.EmRoleEnum.CDV)
        .silentInstallChosenFeatures(emChosenFeatures)
        .installDir(TasBuilder.WIN_SOFTWARE_LOC+"cdv2")
        .emCollector(mom1Col1Role)
        .emCollector(mom2Col1Role)
        .emPort(5002).emWebPort(8082).wvEmPort(8090)
        .nostartEM().nostartWV().build();
        ...
        EmRole standaloneEmRole =
            new EmRole.Builder(STANDALONE_EM_ROLE_ID, tasResolver)
                .nostartEM().nostartWV().build(); 
        ...
        TestbedMachine cdvMachine =
            TestBedUtils
                .createWindowsMachine(CDV_MACHINE_ID, EM_TEMPLATE_ID_WIN, cdv1Role, cdv2Role); 
        
        // Configuration of Cluster1 machine
        TestbedMachine mom1Machine =
            TestBedUtils
                .createWindowsMachine(MOM1_MACHINE_ID, EM_TEMPLATE_ID_WIN, mom1Role, mom1Col1Role);

        // Configuration of Cluster2 machine
        TestbedMachine mom2Machine =
            TestBedUtils
                .createWindowsMachine(MOM2_MACHINE_ID, EM_TEMPLATE_ID_WIN, mom2Role, mom2Col1Role);
        
        // Configuration of Agent Machine
        TestbedMachine agentMachine =
            TestBedUtils.createWindowsMachine(AGENT_MACHINE_ID, AGENT_MACHINE_TEMPLATE_ID_WIN,
