 ...
 */
@TestBedDefinition
public class ConfigurableConfigDirectoryWindowsTestbed implements ITestbedFactory {
    ...
    public ITestbed create(ITasResolver tasResolver) {
        ...
        // create EM1 role
        EmRole em1Role =
            new EmRole.Builder(ConfigurableConfigDirectoryConstants.EM1_ROLE_ID, tasResolver).installDir(ConfigurableConfigDirectoryConstants.EM1_INSTALL_WIN_DIR).nostartEM().nostartWV()
                .build();
        
        // create EM2 role
        EmRole em2Role =
            new EmRole.Builder(ConfigurableConfigDirectoryConstants.EM2_ROLE_ID, tasResolver) 
        		.installDir(ConfigurableConfigDirectoryConstants.EM2_INSTALL_WIN_DIR)
                .silentInstallChosenFeatures(em2chosenfeatures)
                .dbhost(tasResolver.getHostnameById(ConfigurableConfigDirectoryConstants.EM1_ROLE_ID)).nostartEM()
                .nostartWV().build();
        
        // map roles to machines
        ITestbedMachine emMachine =
            TestBedUtils.createWindowsMachine(ConfigurableConfigDirectoryConstants.EM_MACHINE_ID,
            		ConfigurableConfigDirectoryConstants.WINDOWS_TEMPLATE_ID);
