 ...
 */
@TestBedDefinition
public class DomainPermissionsLinuxStandaloneTestbed implements ITestbedFactory {
    ...
    public ITestbed create(ITasResolver tasResolver) {
	  ...
	  //creates Generic roles to download artifacts
		GenericRole downloadDomainsFileRole = new GenericRole.Builder(DOMAIN_FILE_ROLE_ID, tasResolver) 
        .download(new DefaultArtifact("com.ca.apm.coda.testdata.em.virtualagents", "domains", "xml", CONFIG_FILES_ARTIFACT_VERSION), CONFIG_FILES_LOC+"domains.xml")
        .build();
        GenericRole downloadUsersFileRole = new GenericRole.Builder(USER_FILE_ROLE_ID, tasResolver)
        .download(new DefaultArtifact("com.ca.apm.coda.testdata.em.virtualagents", "users", "xml", CONFIG_FILES_ARTIFACT_VERSION), CONFIG_FILES_LOC+"users.xml")
        .build();
        GenericRole downloadAgentClustersFileRole = new GenericRole.Builder(AGENTCLUSTERS_FILE_ROLE_ID, tasResolver)
        .download(new DefaultArtifact("com.ca.apm.coda.testdata.em.virtualagents", "agentclusters", "xml", CONFIG_FILES_ARTIFACT_VERSION), CONFIG_FILES_LOC+"agentclusters.xml")
        .build();
      ...
      //create QAApp role for Tomcat   
        ...
        emMachine.addRole(emRole,downloadDomainsFileRole,downloadUsersFileRole,downloadAgentClustersFileRole);
        ITestbedMachine agentMachine = TestBedUtils.createLinuxMachine(AGENT_MACHINE_ID, AGENT_MACHINE_TEMPLATE_ID);
