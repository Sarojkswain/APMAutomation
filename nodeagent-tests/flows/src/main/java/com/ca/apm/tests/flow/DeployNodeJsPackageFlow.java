...
public class DeployNodeJsPackageFlow extends FlowBase {
	...
	protected void installPackage(File npmPackage) throws IOException {
		...
		if (context.shouldNativeBuildFail()) {
			// one way to cause native module build failure is - mess up python
			// home
			// add operation throws UnsupportedOperationException
			// arguments.add(1, "--python=\\tmp\\pythonX");

			arguments = Arrays.asList("install", "--python=\\tmp\\pythonX",
			        npmPackage.getCanonicalPath());
		}
		performNpmCommand(arguments);
	}
