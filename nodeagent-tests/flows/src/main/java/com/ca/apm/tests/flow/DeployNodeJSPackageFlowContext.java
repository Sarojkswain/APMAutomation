...
public class DeployNodeJSPackageFlowContext implements IFlowContext {
	...
	private boolean shouldNativeBuildFail;
	...
	protected DeployNodeJSPackageFlowContext(Builder builder) {
		...
		this.shouldNativeBuildFail = builder.shouldNativeBuildFail;
	}
	...
	public boolean shouldNativeBuildFail() {
		return shouldNativeBuildFail;
	}
	...
	public static class Builder extends BuilderBase<Builder, DeployNodeJSPackageFlowContext> {
		...
		protected String collectorAgentHost = DEFAULT_COLLAGENT_HOST;
		protected String collectorAgentPort = CollectorAgentProperty.TCP_PORT.getDefaultValue();
		protected boolean shouldNativeBuildFail = false;
		
		
		// install options
		// artifactory tar.gz file location
		protected URL installerUrl;
		
		// published npm package information
		...
		/**
		 * set the location where probe should be installed
		 * 
		 * @param installDir
		 * @return
		 */
