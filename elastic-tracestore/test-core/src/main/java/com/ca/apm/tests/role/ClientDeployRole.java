...
public class ClientDeployRole extends AbstractRole {
	...
	public void deploy(IAutomationAgentClient aaClient) {
		...
		updateDeepTTCountAndCreateBatch(aaClient);
	...
	private void createStressLoadBatchFile(IAutomationAgentClient aaClient) {
		String parameters = "/stressapp/StressApp.jar doSQL=true,doWideStructure=true,doErrors=true,maxAppIndex=250,maxMetricIndex=25,stackDepth=50,maxBackendIndex=20,testDuration=1209600000,threadServiceMaxThreads=100,numberOfConcurrentUsers=50";
		Collection<String> createBatch = Arrays
				.asList("java -javaagent:"
						...
						+ TasBuilder.WIN_SOFTWARE_LOC
						+ "/stressapp/lib/* -Xms256m -Xmx1024m -XX:PermSize=20m -XX:MaxPermSize=30m -XX:+UseSerialGC -XX:+HeapDumpOnOutOfMemoryError -verbosegc -XX:+PrintGCDateStamps -XX:+PrintGCTimeStamps -XX:+PrintGCDetails -Xloggc:"
						+ TasBuilder.WIN_SOFTWARE_LOC
	...
	private void updateDeepTTCountAndCreateBatch(IAutomationAgentClient aaClient) {
		...
		FileModifierFlowContext BatchFile = new FileModifierFlowContext.Builder()
				.create(TasBuilder.WIN_SOFTWARE_LOC + "/deepTT4000.bat",
						createBatch).build();
		runFlow(aaClient, FileModifierFlow.class, BatchFile);
	}
	...
	private void deployJmeter(IAutomationAgentClient aaClient) {
		...
		Collection<String> createBatch = Arrays.asList("cd "
				...
				+ TasBuilder.WIN_SOFTWARE_LOC + "/jmeter/TT_Viewer.JMX -JHOST="
				+ emHost + " -l testresults.csv");
		FileModifierFlowContext BatchFile = new FileModifierFlowContext.Builder()
