...
public abstract class MyEnvPropertySerializerAbs<T> extends AbstractEnvPropertySerializer<T> {
    ...
    public MyEnvPropertySerializerAbs(
        Class<? extends AbstractEnvPropertySerializer<T>> serializerClass) {
    ...
    protected <E> List<E> deserializeListNotNull(Map deserializedMap, String key, Class<E> type) {
        ...
        if (returnObject == null) {
            throw new IllegalArgumentException("Insufficient arguments in env property file: "
                + key + " is missing.");
        }
        return returnObject;
    }
    ...
    protected Object deserializeObject(Map deserializedMap, String key) {
        try (ByteArrayInputStream in =
            new ByteArrayInputStream(Base64.decodeBase64(((String) deserializedMap.get(key))
                .getBytes()));
