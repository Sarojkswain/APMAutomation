...
/**
 * Transaction trace viewer tests.
 *
 * @author haiva01
 * Updated - ahmal01
 */
public class TransactionTraceViewerTest extends UITest {
    private static final Logger log = LoggerFactory.getLogger(TransactionTraceViewerTest.class);
    ...
    private static void verifyMaxDurationFilter(SearchContext transactionTraceListViewer,
        SearchContext table) {
        WebElement avgLabel = transactionTraceListViewer.findElement(By.xpath("//*[contains(@id, 'avgLabel')]"));
        String avgValueText = parseAvgDurationLabel(avgLabel);
        final int avg = Integer.parseInt(avgValueText);

        WebElement maxDurationInput = transactionTraceListViewer.findElement(By.xpath("//*[contains(@id, 'txtMaxDuration')]"));
        
        maxDurationInput.clear();
        maxDurationInput.sendKeys(avgValueText);
        Utils.sleep(2000);
        ...
        Collection<String> durationTexts = collectTextFromElements(durationTextElements);
        List<Integer> durationValues = extractDurationValues(durationTexts);
        assertTrue(Iterables.all(durationValues, new Predicate<Integer>() {
            @Override
    ...
    private static void verifyMinDurationFilter(SearchContext transactionTraceListViewer,
        SearchContext table) {
        WebElement avgLabel = transactionTraceListViewer.findElement(By.xpath("//*[contains(@id, 'avgLabel')]"));
        String avgValueText = parseAvgDurationLabel(avgLabel);
        final int avg = Integer.parseInt(avgValueText);

        WebElement minDurationInput = transactionTraceListViewer.findElement(By.xpath("//*[contains(@id, 'txtMinDuration')]"));
        
        minDurationInput.clear();
        minDurationInput.sendKeys(avgValueText);
        Utils.sleep(2000);
    ...
    private void verifyTransactionTracesTable(UI ui, String boxName, SearchContext tableParent,
        ...
        verifyMaxDurationFilter(transactionTraceListViewer, table);
        verifyMinDurationFilter(transactionTraceListViewer, table);
    
        verifyErrorFilter(transactionTraceListViewer, transactionTraceListFilterBar);
    ...
    private static void verifyErrorFilter(PageElement transactionTraceListViewer,
        PageElement transactionTraceListFilterBar) {
        PageElement errorFilterElement = transactionTraceListFilterBar.findElement(
            By.xpath("//*[contains(@id, 'chkErrorsFilter')]"));
        errorFilterElement.click();
        Utils.sleep(1000);
        Collection<PageElement> barElements = transactionTraceListViewer.findPageElements(
            By.cssSelector("div.tt-instance-bar"));
        for (PageElement pageElement : barElements) {
            String classValue = pageElement.getAttribute("class");
            Matcher matcher = matchRegExAndLog(".*\\berror\\b.*", classValue);
            assertNotNull(matcher, "CSS class 'error' was not found on this bar");
        }
        errorFilterElement.click();
        Utils.sleep(1000);
    }
    ...
    @Tas(testBeds = @TestBed(name = StandAloneTestBed.class,
        executeOn = StandAloneTestBed.SA_MASTER), size = SizeType.MEDIUM, owner = "haiva01")
    @Test(groups = {"ttviewer_ui"})
    public void testInMapViewThroughDrawerHandle() {
        UI ui = getUI();
        try {
            ui.login();
        } catch (Throwable e) {
        ...
        final String perspective = "No Group";
        try {
        ...
        try {
            String boxName = "User_Story_Updated via iOS 9";
            Canvas canvas = initialMapNavigation(ui, boxName);

            WebElement bottomDrawerButton = Utils.waitForCondition(ui.getDriver(),
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(@id, 'bottomDrawerMaximizeButton')]")),
                60);
            new Actions(ui.getDriver())
                .moveToElement(bottomDrawerButton)
                .click()
                .perform();
            ...
            WebElement bottomDrawerContent = Utils.waitUntilVisible(
                ui.getDriver(), By.xpath("//*[contains(@id, 'bottom-drawer-content')]"));
    ...
    @Tas(testBeds = @TestBed(name = StandAloneTestBed.class,
        executeOn = StandAloneTestBed.SA_MASTER), size = SizeType.MEDIUM, owner = "haiva01")
    @Test(groups = {"ttviewer_ui"})
    public void testInMapViewThroughComponentViewLink() {
        UI ui = getUI();
        try {
            ui.login();
        } catch (Throwable e) {
    ...
    @Tas(testBeds = @TestBed(name = StandAloneTestBed.class,
        executeOn = StandAloneTestBed.SA_MASTER), size = SizeType.MEDIUM, owner = "haiva01")
    @Test(groups = {"ttviewer_ui"})
    public void testInExperienceView() {
        UI ui = getUI();
        try {
            ui.login();
        } catch (Throwable e) {
