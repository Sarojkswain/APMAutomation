...
import com.ca.apm.test.em.util.RoleUtility.NowhereBankLoad;
...
public class StandAloneTestBed implements ITestbedFactory {
    ...
    public static final Logger log = LoggerFactory.getLogger(StandAloneTestBed.class);
    ...
    public static final String NOWHEREBANK_MACHINE = "nowhereBankMachineId";
    ...
    public static final String SELENIUM_HUB_ROLE_ID = SeleniumGridMachinesFactory.HUB_ROLE_ID;
    public static final String SELENIUM_GRID_ROLE_ID = SeleniumGridMachinesFactory.NODE_ROLE_ID;
    public static final String SELENIUM_GRID_MACHINE_ID = TOMCAT_AGENT_ONLY;// "seleniumGridMachineId";
    public static final String SELENIUM_HUB_MACHINE_ID = SA_MASTER;// "seleniumHubMachineId";
    ...
    public ITestbed create(ITasResolver tasResolver) {
        ...
        testbed.addMachine(tomcatAgentOnlyMachine);
        ...
        testbed.addMachine(wlsAgentOnlyMachine);
        ...
        // Nowhere Bank Machine
        ITestbedMachine nowhereBankMachine =
            TestBedUtils.createWindowsMachine(NOWHEREBANK_MACHINE, TEMPLATE_W64);
        IRole lastNbRole =
            RoleUtility.addNowhereBankRole(nowhereBankMachine, saMaster, null,
                new NowhereBankLoad[] {NowhereBankLoad.SlowWebService, NowhereBankLoad.DBFailing},
                tasResolver);
        IRole delayRole =
            new DelayRole("delayAfterNowhereBankStart", (int) TimeUnit.MINUTES.toSeconds(2));
        delayRole.after(lastNbRole);
        nowhereBankMachine.addRole(delayRole);
        RoleUtility.addMmRole(saMasterMachine, saMaster.getRoleId() + "_mm", saMaster,
            "NowhereBankMM");
        testbed.addMachine(nowhereBankMachine);
        ...
        testbed.addProperty("test.applicationBaseURL",
            "http://" + tasResolver.getHostnameById(saMaster.getRoleId()) + ":8082/ApmServer");
    ...
    private SeleniumGridNodeRole createSeleniumNodeRole(ITasResolver tasResolver, String machineId,
        String hubHost) {
        log.info("Using HUB host {}", hubHost);
        URL hubUrl = null;
        try {
            hubUrl = new URL("http", hubHost, 4444, "/grid/register/");
        } catch (MalformedURLException ex) {
            String msg =
                MessageFormat.format("HUB URL is malformed. Exception: {0}", ex.getMessage());
            log.error(msg, ex);
        ...
        NodeCapability firefoxCapability =
            new NodeCapability.Builder().browserType(BrowserType.FIREFOX)
                .platform(NodePlatform.WINDOWS).maxInstances(1).build();
        ...
        NodeConfiguration nodeConfiguration =
            new NodeConfiguration.Builder().addCapability(firefoxCapability)
                .addCapability(internetExplorerCapability).addCapability(chromeCapability)
                .maxSession(100).hub(hubUrl).build();

        return new SeleniumGridNodeRole.Builder(SELENIUM_GRID_ROLE_ID, tasResolver)
            .nodeConfiguration(nodeConfiguration)
            .standaloneServerVersion(SeleniumStandaloneServer.V2_53_0)
            // .chromeDriver(SeleniumChromeDriver.V2_24_B32)
            // .internetExplorerDriver(SeleniumInternetExplorerDriver.V2_53_1_B64)
            .build();
    }
