...
/**
 * Base class for BrowserAgent Automation
 *
...
import com.ca.apm.tests.utils.constants.MetricConstants.NoBTMetricPath;
import com.ca.apm.tests.utils.constants.TestAppUrlConstants.BrtmTestApp;
import com.ca.apm.tests.utils.constants.AgentPropertyConstants;
...
public class BrowserAgentBaseTest extends TasTestNgTest {
    ...
    protected void testSuiteSetUp() {
        ...
        agent =
            new AgentDetails.Builder()
                ...
                .transactionTraceDirectory(automationBaseDir)
    ...
    /**
     * Reset agent profile properties and browser.
     */
    @BeforeMethod(alwaysRun = true)
    protected void prepareTest() throws MalformedURLException {
        LOGGER.info("In prepareTest");
        resetProps();
        CommonUtils.closeBrowser(seleniumData);
        CommonUtils.createWebDriver(seleniumData);
        LOGGER.info("End prepareTest");
    }
     ...
     * Closes the selenium and the browser.
     */
    @AfterClass(alwaysRun = true)
    protected void tearDown() {
        CommonUtils.writeBrowserLogToFile(seleniumData);
        try {
            LOGGER.info("####################################\n\n\n");
            LOGGER.info("Total Time spent in Sleep: " + totalSleepTime + " milli seconds or "
                + totalSleepTime / 1000 + " seconds");
            // TODO: This code needs to be reviewed - MetricTime is not updated by other methods
            LOGGER.info("Total Time spent in getiing metrics from EM: " + totalMetricTime * 1000
                + " milli seconds or " + totalMetricTime + " seconds");
            totalExecutionTime.stop();
            LOGGER.info("Total Time for test Execution: " + totalExecutionTime.getElapsedTime()
                + " milli seconds or " + totalExecutionTime.getElapsedTimeSecs() + " seconds");
            LOGGER.info("\n\n\n####################################");
            seleniumData.getDriver().close();
            seleniumData.getDriver().quit();
