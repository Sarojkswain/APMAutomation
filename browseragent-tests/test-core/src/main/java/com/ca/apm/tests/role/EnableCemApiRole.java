...
public class EnableCemApiRole extends AbstractRole {
    ...
    protected EnableCemApiRole(Builder builder) {
        ...
        this.unpackDir = TasBuilder.WIN_SOFTWARE_LOC + "/cemtemp";
    ...
    private void unpackJar(IAutomationAgentClient aaClient) {
        ...
        String jarFilePath =
            emHomeDir + "/product/enterprisemanager/plugins/com.wily.apm.tess_"
                + emInstallVersion.replace("-SNAPSHOT", "") + ".jar";

        JarArchiveFlowContext context =
            new JarArchiveFlowContext.Builder().archivePath(jarFilePath).tempUnpackDir(unpackDir)
                .unpack(true).build();
    ...
    private void modifyXml(IAutomationAgentClient aaClient) {
        ...
        FileModifierFlowContext context =
            new FileModifierFlowContext.Builder().replace(xmlPath, replacePairs).build();
