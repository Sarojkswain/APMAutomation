 ...
 */
@TestBedDefinition
public class QATestAppEmTestBed implements ITestbedFactory {
    ...
    public static final Collection<String> EM_LAXNL_JAVA_OPTION = Arrays.asList("-Dappmap.token="
        + ADMIN_AUX_TOKEN, "-Dappmap.user=" + ADMIN, "-Dcom.wily.assert=false",
        "-XX:+HeapDumpOnOutOfMemoryError", "-verbosegc", "-Xdebug",
        "-Xrunjdwp:server=y,transport=dt_socket,address=20555,suspend=n");

    @Override
    public ITestbed create(ITasResolver tasResolver) {
        ITestbed testbed = new Testbed(QATestAppEmTestBed.class.getSimpleName());
        /* -- Initialize machines -- */
        ITestbedMachine emMachine =
            TestBedUtils.createWindowsMachine(MACHINE_ID_EM, ITestbedMachine.TEMPLATE_W64);
        /* -- Declare roles -- */
        EmRole emRole;
        AgentRole agentRole;
        ...
        emRole =
            new EmRole.Builder(ROLE_ID_EM, tasResolver)
        ...
        DefaultArtifact qaTestAppArtifact =
            new DefaultArtifact(QA_TEST_APP_GROUP_ID, QA_TEST_APP_ARTIFACT_ID, "jvm7-tomcatnodb",
                "war", QA_TEST_APP_VERSION);

        webAppRole =
            new WebAppRole.Builder<TomcatRole>(ROLE_ID_QA_TEST_APP).artifact(qaTestAppArtifact)
                .cargoDeploy().contextName(CONTEXT_NAME_QA_TA).build();

        emMachine.getDefaultJavaHome();

        tomcatRole =
            new TomcatRole.Builder(ROLE_ID_TOMCAT, tasResolver).tomcatVersion(TomcatVersion.v70)
                .webApp(webAppRole).build();
        ...
        agentRole =
            new AgentRole.Builder(ROLE_ID_AGENT, tasResolver).version(AGENT_VERSION)
                .additionalProperties(agentAdditionalProps).webAppServer(tomcatRole).emRole(emRole)
                .build();
