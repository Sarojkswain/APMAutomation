...
public class DXCRole extends AbstractRole {
	...
	public static final String DXC_VERSION = "2.3.0.3";
	public static final String BA_VERSION = "2.3.0.3";
    ...
    private final RunCommandFlowContext startDxcFlowContext;
    private final RunCommandFlowContext stopDxcFlowContext;
    private final RunCommandFlowContext getJsonFlowContext;
    private final RunCommandFlowContext uploadBaFlowContext;
    private final RunCommandFlowContext dxcStatusFlowContext;
	...
	protected DXCRole(Builder builder) {
        ...
        this.startDxcFlowContext = builder.startDxcFlowContext;
        this.stopDxcFlowContext = builder.stopDxcFlowContext;
        this.getJsonFlowContext = builder.getJsonFlowContext;
        this.uploadBaFlowContext = builder.uploadBaFlowContext;
        this.dxcStatusFlowContext = builder.dxcStatusFlowContext;
	}
	...
	public RunCommandFlowContext getStartDxcFlowContext() {
        return startDxcFlowContext;
    }

    public RunCommandFlowContext getStopDxcFlowContext() {
        return stopDxcFlowContext;
    }

    public RunCommandFlowContext getGetJsonFlowContext() {
        return getJsonFlowContext;
    }

    public RunCommandFlowContext getUploadBaFlowContext() {
        return uploadBaFlowContext;
    }

    public RunCommandFlowContext getDxcStatusFlowContext() {
        return dxcStatusFlowContext;
    }
    ...
    private void deployDXC(IAutomationAgentClient aaClient) {
        ...
        Collection<String> data =
            Arrays.asList("(tar -xvf " + installDir + "DigitalExperienceCollector* -C "
                ...
                + "DigitalExperienceCollector*/* " + installDir + ") &&" + " ( rm -rf "
                + installDir + "DigitalExperienceCollector* )");

        FileModifierFlowContext makeFileModification =
            new FileModifierFlowContext.Builder().create("/opt/moveDxcFiles.sh", data).build();
        runFlow(aaClient, FileModifierFlow.class, makeFileModification);
        RunCommandFlowContext command =
            new RunCommandFlowContext.Builder("moveDxcFiles.sh").workDir("/opt").build();
	...
	private void moveBAExtJsFile(IAutomationAgentClient aaClient) {
		 ...
		 FileModifierFlowContext updateDirContext =
		            new FileModifierFlowContext.Builder().move(
		                installDir +"browserAgent/wa/ba/default/BAExt.js" ,
		                installDir + "browserAgent/wa/ba/profile/default-tenant/default-app/BAExt.js").build();
	...
	private void unJarJsons(IAutomationAgentClient aaClient) {
		Collection<String> data = Arrays
				.asList("unzip /tmp/tests/com/ca/tas/dxc/dxc-testing-core/" + BA_VERSION 
						+ "/dxc-testing-core-" + BA_VERSION + "-jar-with-dependencies.jar -d /opt/unjar");
		FileModifierFlowContext createFileFlow = new FileModifierFlowContext.Builder()
				.create("/tmp/unjar.sh", data).build();
		runFlow(aaClient, FileModifierFlow.class, createFileFlow);
	}

	private void configureCollector(IAutomationAgentClient aaClient) {
		InetAddress ipAddr;
		try {
	...
	public static class Builder extends BuilderBase<Builder, DXCRole> {
        ...
        private RunCommandFlowContext startDxcFlowContext;
        private RunCommandFlowContext stopDxcFlowContext;
        private RunCommandFlowContext getJsonFlowContext;
        private RunCommandFlowContext uploadBaFlowContext;
        private RunCommandFlowContext dxcStatusFlowContext;
		...
		private void startDXC() {
		    startDxcFlowContext = new RunCommandFlowContext.Builder(
					"startDXC.sh").workDir(installDir).build();
			getEnvProperties().add(DXC_START, startDxcFlowContext);
		}

		private void stopDXC() {
		    stopDxcFlowContext = new RunCommandFlowContext.Builder(
					"stopDXC.sh").workDir(installDir).build();
			getEnvProperties().add(DXC_STOP, stopDxcFlowContext);
		}

		private void getJson() {
		    getJsonFlowContext = new RunCommandFlowContext.Builder(
					"unjar.sh").workDir("/tmp").build();
			getEnvProperties().add(GET_JSON, getJsonFlowContext);
		}

		private void uploadBAFiles() {
		    uploadBaFlowContext = new RunCommandFlowContext.Builder(
					"runUploadBA.sh").workDir(installDir).build();
			getEnvProperties().add(UPLOAD_BA, uploadBaFlowContext);
		}

		private void dxcStatusFile() {
		    dxcStatusFlowContext = new RunCommandFlowContext.Builder(
					"dxcStatus.sh").workDir(installDir).build();
			getEnvProperties().add(DXC_STATUS, dxcStatusFlowContext);
		}
