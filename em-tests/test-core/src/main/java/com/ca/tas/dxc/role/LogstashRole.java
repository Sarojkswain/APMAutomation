...
public class LogstashRole extends AbstractRole {
    ...
    private final RunCommandFlowContext startLogstashFlowContext;
    private final RunCommandFlowContext stopLogstashFlowContext;
    private final RunCommandFlowContext startAPMLogstashFlowContext;
	...
	protected LogstashRole(Builder builder) {
        ...
        this.startLogstashFlowContext = builder.startLogstashFlowContext;
        this.stopLogstashFlowContext = builder.stopLogstashFlowContext;
        this.startAPMLogstashFlowContext = builder.startAPMLogstashFlowContext;
	}
	...
	public RunCommandFlowContext getStartLogstashFlowContext() {
        return startLogstashFlowContext;
    }

    public RunCommandFlowContext getStopLogstashFlowContext() {
        return stopLogstashFlowContext;
    }

    public RunCommandFlowContext getStartAPMLogstashFlowContext() {
        return startAPMLogstashFlowContext;
    }
    ...
    private void configureLogstash(IAutomationAgentClient aaClient) {
		...
		Collection<String> data1 = Arrays.asList("input {", "kafka {",
				...
				"topic_id => 'analytics'", "}", "}", "output {","stdout {", "codec => rubydebug ","}",
				"elasticsearch { hosts => [\"" + elasticHost + ":"
						+ elasticPort + "\"]",
	...
	private void configureAPMLogstash(IAutomationAgentClient aaClient) {	
		...
		updateArgs.put(
				"agentManager.url.1=(.*)",
				"agentManager.url.1=" +emHost + ":"
						+ Integer.toString(emPort));
	...
	private void installAPMLogstashPlugin(IAutomationAgentClient aaClient) {
		...
		// Install plugin
		Collection<String> data = Arrays.asList(installDir
				+ "/bin/logstash-plugin install " + installDir
				+ "/bin/logstashplugin.gem");
		FileModifierFlowContext createFileFlow = new FileModifierFlowContext.Builder()
	...
	private void logstashDeploy(IAutomationAgentClient aaClient) {
        ...
        Collection<String> data =
            Arrays.asList("(tar -xvf " + installDir + "DigitalExperienceCollector* -C "
                ...
                + "DigitalExperienceCollector*/logstash/* " + installDir + ") &&" + " ( rm -rf "
                + installDir + "DigitalExperienceCollector* ) &&" + " ( tar -xvf " + installDir
                + "logstash*tgz -C " + installDir + ") &&" + " ( mv -v " + installDir
                + "logstash*/* " + installDir + ") &&" + " ( rm -rf " + installDir
                + "logstash*) &&" + " ( mv " + installDir + "apm-logstash-plugin/agent.properties "
                + installDir + "apm-logstash-plugin/dxc-logstash-apm-template.conf " + installDir
                + "bin) &&" + " ( mv " + installDir + "apm-logstash-plugin/logstash-output*gem "
	...
	public static class Builder extends BuilderBase<Builder, LogstashRole> {
        ...
        private RunCommandFlowContext startLogstashFlowContext;
        private RunCommandFlowContext stopLogstashFlowContext;
        private RunCommandFlowContext startAPMLogstashFlowContext;
		...
		private void startLogstash() {
		    startLogstashFlowContext = new RunCommandFlowContext.Builder(
					"startLogstash.sh").workDir(installDir)
					.terminateOnMatch("Pipeline main started").build();
			getEnvProperties().add(LOGSTASH_START, startLogstashFlowContext);

		}

		private void stopLogstash() {
		    stopLogstashFlowContext = new RunCommandFlowContext.Builder(
					"stopLogstash.sh").workDir(installDir)
					.terminateOnMatch("Pipeline main started").build();
			getEnvProperties().add(LOGSTASH_STOP, stopLogstashFlowContext);

		}

		private void startAPMLogstash() {
		    startAPMLogstashFlowContext = new RunCommandFlowContext.Builder(
					"startAPMLogstash.sh").workDir(installDir)
					.terminateOnMatch("Pipeline main started").build();
			getEnvProperties().add(LOGSTASH_APM_START, startAPMLogstashFlowContext);
