...
public class DotNetAgentDeployRole extends AbstractRole {
    ...
    private void configureLegacyAgent(IAutomationAgentClient aaClient) {
        ...
        if(isLegacyMode) { 
            FileModifierFlowContext copyFiles = new FileModifierFlowContext.Builder()
                .copyFiltered(installDir + "/wily/examples/legacy", installDir + "/wily",
                new TasFileNameFilter(".*", TasFileNameFilter.FilterMatchType.REGULAR_EXPRESSION))
                .build();
    ...
    private void deployArtifact(IAutomationAgentClient aaClient) {
        ...
        URL url = tasResolver.getArtifactUrl(new DefaultArtifact("com.ca.apm.delivery", 
            artifact, "64", "zip", AgentRegressionBaseTestBed.getAgentArtifactVersion(tasResolver)));            
