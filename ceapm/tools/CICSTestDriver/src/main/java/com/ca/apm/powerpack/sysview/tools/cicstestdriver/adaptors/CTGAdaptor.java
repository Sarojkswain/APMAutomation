...
public class CTGAdaptor extends GenericAdaptor
{
	private static final Logger LOGGER = Logger.getLogger(CTGAdaptor.class);
    ...
    private static String CLIENT_PROGRAM_CHANNEL = "CLIENT_PROGRAM_CHANNEL";
    private static String CLIENT_PROGRAM_CONTAINER = "CLIENT_PROGRAM_CONTAINER";
    private static String CLIENT_PROGRAM_PROP = "CLIENT_PROGRAM_PROP";
    
    // This needs to match JavaGatewayFlowPerThreadTracer class value
	...
	public boolean initialize(CallJobStack inCallJobStack)
	{
		...
		LOGGER.info("Connecting to CTG server gate: '" + inCallJobStack.getJGate() +
		            ...
		            (inCallJobStack.isUseCommarea() ? "useCommarea" : "")+ 
		            (inCallJobStack.isUseChannel() ? "useChannel": "useNothing") + 
	...
	private void RunExtendedTransaction(int threadNumber, CalledTransaction calledTransaction)
	{
		...
		// Add the channel if need to
		reqChannel = addChannel(threadNumber);
		...
		try
		{
            ...
            validateReturnedChannel(threadNumber, reqChannel);
		}
		catch(Exception ex)
		{
	       	LOGGER.error("#"+threadNumber+":"+ 
	...
	private Channel addChannel(int threadNumber)
	{
	    Channel reqChannel = null;
	    
	    // If using channel then add one
        if (callJobStack.isUseChannel())
        {
            try
            {
                reqChannel = new Channel(CLIENT_PROGRAM_CHANNEL);
                reqChannel.createContainer(CLIENT_PROGRAM_CONTAINER, CLIENT_PROGRAM_PROP);
                eciRequest.setChannel(reqChannel);
            }
            catch(ChannelException e)
            {
                LOGGER.error("#"+threadNumber+":"+ 
                             "Could not create channel "+ CLIENT_PROGRAM_CHANNEL + " from CTGAdaptor ");
            }
            catch(UnsupportedEncodingException e)
            {
                LOGGER.error("#"+threadNumber+":"+ 
                             "Could not create container "+ CLIENT_PROGRAM_CONTAINER + " from CTGAdaptor because of encoding error on:" + CLIENT_PROGRAM_PROP);
            }
            catch(ContainerException e)
            {
                LOGGER.error("#"+threadNumber+":"+ 
                             "Could not create container "+ CLIENT_PROGRAM_CONTAINER + " from CTGAdaptor");
            }
        }	
        return reqChannel;
	}
     ...
     */
    private void validateReturnedChannel(int threadNumber, Channel reqChannel) 
    {
    	try
	    {
	        // Validate the returned channel
	        Channel returnedChannel = eciRequest.getChannel();
	        if (reqChannel != returnedChannel)
	        {
	            LOGGER.error("#"+threadNumber+":"+ 
	                         "Recieved back different channel object than sent.");
	        }
	        // Continue validating channel
	        if (callJobStack.isUseChannel())
	        {
	            Container returnedContainer = null;
	            
	            // If we had a channel
	            if (returnedChannel != null)
	            {
	                // If this is the tracer injected channel then give warning
	                if (returnedChannel.getName().equals(SYSVIEW_INTEGRATION_CHANNEL))
	                {
	                    LOGGER.error("#"+threadNumber+":"+ 
	                                 "Channel, '"+ SYSVIEW_INTEGRATION_CHANNEL +"', was left in by the tracer. This should not be a problem because application will not know to fetch it and should ignore it.");
	                }
	                
	                try
	                {
	                    returnedContainer = returnedChannel.getContainer(CLIENT_PROGRAM_CONTAINER);
	                    if (returnedContainer != null)
	                    {
	                        // Make sure sent property is still there
	                        try
	                        {
	                            String returnedContainerData = returnedContainer.getCHARData();
	                            if (!returnedContainerData.equals(CLIENT_PROGRAM_PROP))
	                            {
	                                LOGGER.error("#"+threadNumber+":"+ 
	                                             "Recieved back different container data than was sent.  Sent:[" +CLIENT_PROGRAM_PROP+ "] Recieved:["+returnedContainerData+"]");
	                            }
	                        }
	                        catch(UnsupportedEncodingException e)
	                        {
	                            LOGGER.error("#"+threadNumber+":"+ 
	                                         "Recieved back UnsupportedEncodingException on container data than was sent.  Sent:[" +CLIENT_PROGRAM_PROP+ "]");
	                        }
	                        catch(ContainerException e)
	                        {
	                            LOGGER.error("#"+threadNumber+":"+ 
	                                         "Container, '"+ CLIENT_PROGRAM_PROP +"', that was sent was not recieved back. ContainerException");
	                        }
	                    }
	                    else
	                    {
	                        LOGGER.error("#"+threadNumber+":"+ 
	                                     "Container, '"+ CLIENT_PROGRAM_CONTAINER +"', that was sent was not recieved back.");
	                    }
	                }
	                ...
	                try
	                {
	                    // Make sure that tracer didn't leave decoration containers in
	                    returnedContainer = returnedChannel.getContainer(SYSVIEW_INTEGRATION_GUID);
	                    if (returnedContainer != null)
	                    {
	                        LOGGER.info("#"+threadNumber+":"+ 
	                                     "Container, '"+ SYSVIEW_INTEGRATION_GUID +"', was left in by the tracer.  This should not be a problem because application will not know to fetch it and should ignore it.");
	                ...
	                {
	                    // Make sure that tracer didn't leave decoration containers in
	                    returnedContainer = returnedChannel.getContainer(SYSVIEW_SEQUENCE_ID_NAME);
	                    if (returnedContainer != null)
	                    {
	                        LOGGER.info("#"+threadNumber+":"+ 
	                                     "Container, '"+ SYSVIEW_SEQUENCE_ID_NAME +"', was left in by the tracer.");
	                ...
	                {
	                    // Make sure that tracer didn't leave decoration containers in
	                    returnedContainer = returnedChannel.getContainer(SYSVIEW_TT_PROP_FLAG);
	                    if (returnedContainer != null)
	                    {
	                        LOGGER.info("#"+threadNumber+":"+ 
	                                     "Container, '"+ SYSVIEW_TT_PROP_FLAG +"', was left in by the tracer.  This should not be a problem because application will not know to fetch it and should ignore it.");   
	                    }
	                }
	                catch (ContainerNotFoundException ex)
	                {
	            ...
	            // Otherwise there was no returned channel
	            else
	            {
	                LOGGER.error("#"+threadNumber+":"+ 
	                             "Returned channel was null.");
	            }
	        } 
		}
		catch(Exception ex)
		{
	       	LOGGER.error("#"+threadNumber+":"+ 
	                        "Exception validateReturnedChannel " + ex);
	       	ex.printStackTrace();
	    }
    }
